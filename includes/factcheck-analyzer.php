...
class AI_Verify_Factcheck_Analyzer {
    ...
    public static function extract_claims($content) {
        ...
        // Try ClaimBuster first (highly accurate, free)
...
- Confidence should reflect quality of sources (0.9+ for government/academic, 0.7+ for news)";
        ...
        $response = wp_remote_post('https://api.perplexity.ai/chat/completions', array(
            ...
            'body' => json_encode(array(
                'model' => 'llama-3.1-sonar-large-128k-online', // Built-in web search
                'messages' => array(
...
Article: {$content}";
    ...
    public static function analyze_with_single_call_perplexity($content, $context, $combined_context) {
        ...
        if (is_wp_error($response)) {
            error_log('AI Verify: Perplexity single-call error: ' . $response->get_error_message());
            return $response;
        }
    ...
    public static function analyze_with_single_call_openrouter($content, $context, $combined_context) {
        ...
        preg_match('/\{.*\}/s', $body, $matches);
        if (empty($matches[0])) {
            error_log('AI Verify: OpenRouter single-call failed to return valid JSON. Body: ' . $body);
            return new WP_Error('json_error', 'AI failed to produce a valid JSON report.');
        }
